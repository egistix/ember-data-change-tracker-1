...
export const relationShipTransform = {
	belongsTo: {
		serialize(model, key, options) {
			...
			//console.log(relationship.store._store.findRecord(relationship.localState.type, relationship.localState.id));
			let value = relationship.definition.isImplicit ? relationship.identifier : relationship.localState;
